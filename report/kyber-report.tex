...
always lands on whichever node is currently primary --- no manual intervention. This removes
...
\begin{itemize}
  \item \textbf{Reaching things.} Management is VPN-only: bring up the VPN, then SSH to a
        host directly or hop through the router as a jump host. Inside, rely on
        \code{kyber.local} names --- they resolve to the right private addresses.
  \item \textbf{Change management.} The router's running configuration is kept as a redacted
        export --- private keys and password hashes removed --- refreshed after every change,
        so there is always an up-to-date copy that holds no secrets. Each host's build is
        recorded step by step, so any box can be rebuilt from scratch.
  \item \textbf{Secrets.} Service passwords stay in root-owned files on each host and never
        leave it; only public material, such as the CA certificate, is shared.
\end{itemize}

\paragraph{Not built (and why).} A few \emph{optional} features were intentionally left out
of scope, called out here so nobody goes looking for them: \textbf{HTTP/3 (QUIC)} and a
\textbf{GraphQL} mirror of the API, an \textbf{IDS/IPS} (Suricata), and \textbf{public DNS}
(there is no registered domain, so external clients use a local hosts file). Everything else
described above --- segmentation, dual-stack addressing, NAT and NPTv6, DHCP/DHCPv6/SLAAC,
split DNS, NTP, the dual-stack firewall, the LDAP-authenticated VPN, the TLS REST API with
content negotiation and HA, the FreeIPA directory, SNMP-based monitoring with Grafana,
NetFlow analysis, and the RAFT cluster --- is built and running.
